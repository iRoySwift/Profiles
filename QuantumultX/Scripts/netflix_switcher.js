...
(async () => {
  ...
  if (status === STATUS_FULL_AVAILABLE) {
    ...
    $.msg(
      $.name,
      `${actualNode}`,
      `该节点完整支持 Netflix ➟ ${flag} ${regionName}`
    );
    return;
  }
  ...
  try {
    cacheFullPolicies = JSON.parse(
      $.getval("Helge_0x00.Netflix_Full_Available_Policies") ?? "[]"
    );
  } catch (error) {
  ...
  try {
    cacheOriginalPolicies = JSON.parse(
      $.getval("Helge_0x00.Netflix_Original_Available_Policies") ?? "[]"
    );
  } catch (error) {
  ...
  let nodes = new Set(
    paths
      ...
      .filter((item) => !["proxy", "direct", "reject"].includes(item))
  );

  // 检测一遍缓存的可用子策略是否还在当前策略中
  cacheFullPolicies = cacheFullPolicies.filter(
    (item) => nodes.has(item.policy) && item.policy !== selected
  );
  cacheOriginalPolicies = cacheOriginalPolicies.filter(
    (item) => nodes.has(item.policy) && item.policy !== selected
  );
  ...
  if (recheck) {
    let recheckPolicies = [
      ...cacheFullPolicies.map((item) => item.policy),
      ...cacheOriginalPolicies.map((item) => item.policy),
    ];
    let { fullAvailablePolicies, originalAvailablePolicies } =
      await testPolicies(recheckPolicies);
    ...
    if (sortByTime) {
      ...
      cacheOriginalPolicies = cacheOriginalPolicies.sort(
        (m, n) => m.time - n.time
      );
    }
  }
  ...
  $.setval(
    JSON.stringify(cacheFullPolicies),
    "Helge_0x00.Netflix_Full_Available_Policies"
  );
  $.setval(
    JSON.stringify(cacheOriginalPolicies),
  ...
  let {
    ...
    region: newRegion,
    status: newStatus,
  } = cacheFullPolicies.length > 0
    ? cacheFullPolicies[0]
    : cacheOriginalPolicies[0];

  // 找到切换路径，并按照路径长度排序，取路径长度最短的
  let switchPath = paths
    .filter((path) => path[path.length - 1] === newPolicy)
    .sort((m, n) => m.length - n.length)[0];
  let switchDict = {};
  ...
  $.msg(
    $.name,
    `${curPolicyPath[curPolicyPath.length - 1]} ➟ ${
      switchPath[switchPath.length - 1]
    }`,
    msg
  );
})()
    ...
    for (let { policy, status, region, time } of results) {
      switch (status) {
        ...
        case STATUS_ORIGINAL_AVAILABLE: {
          ...
          let regionName = REGIONS?.[region.toUpperCase()]?.chinese ?? "";
          console.log(
            `${policy}: 仅支持 Netflix 自制剧 ➟ ${flag}${regionName}`
          );
      ...
      (response) => {
        ...
        if (statusCode === 302 || statusCode === 301 || statusCode === 200) {
          if (debug) {
            if (statusCode === 200) {
              console.log(
                `${policyName} filmId: ${filmId}, statusCode: ${statusCode}, X-Originating-URL: ${originatingUrl}`
              );
            } else {
              console.log(
                `${policyName} filmId: ${filmId}, statusCode: ${statusCode}, Location: ${location}`
              );
            }
          }
        ...
        if (debug) {
          console.log(
            `${policyName} filmId: ${filmId}, statusCode: ${statusCode}, response: ${JSON.stringify(
              response
            )}`
          );
        }
...
async function test(policyName) {
  ...
  let result = await Promise.race([
    getFilmPage(81215567, policyName),
    timeout(t),
  ])
    .then((region) => {
    ...
    .catch(async (error) => {
      ...
      let region = await Promise.race([
        getFilmPage(80018499, policyName),
        timeout(t),
      ]);
...
function lookupPath(
  ...
  if (
    ...
    targetPolicy?.type === undefined ||
    !Array.isArray(targetPolicy?.candidates)
  ) {
    return paths;
  }
