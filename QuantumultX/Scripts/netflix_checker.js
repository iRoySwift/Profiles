...
(async () => {
  ...
  let { fullAvailablePolicies, originalAvailablePolicies } = await testPolicies(
    policyName,
    candidatePolicies
  );
  if (sortByTime) {
    fullAvailablePolicies = fullAvailablePolicies.sort(
      (m, n) => m.time - n.time
    );
    originalAvailablePolicies = originalAvailablePolicies.sort(
      (m, n) => m.time - n.time
    );
  }
  $.setval(
    JSON.stringify(fullAvailablePolicies),
    "Helge_0x00.Netflix_Full_Available_Policies"
  );
  $.setval(
    JSON.stringify(originalAvailablePolicies),
...
async function testPolicies(policyName, policies = []) {
  ...
  if (retry && failedPolicies.length > 0) {
    await Promise.map(failedPolicies, (subPolicy) => test(subPolicy), {
      concurrency,
    })
      .then(echo)
      .catch((error) => console.log(error));
...
async function test(policyName) {
  ...
  let result = await Promise.race([
    getFilmPage(81280792, policyName),
    timeout(t),
  ])
    .then((region) => {
...
function lookupChildrenNode(policies = {}, targetPolicyName) {
  ...
  while (looking.length > 0) {
    ...
    for (const policy of policies[curPolicyGroupName].candidates) {
      ...
      if (
        !looked.has(policy) &&
        !looking.includes(policy) &&
        policies[policy]?.type === "static"
      ) {
        looking.push(policy);
      }
    }
